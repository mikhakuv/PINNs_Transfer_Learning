...
$$Rel_h = \frac{\sqrt{ \sum\limits_{i=1}^N (|q_{i}^{truth}|-|q_{i}^{pred}|)^2 }}{\sqrt{\sum\limits_{i=1}^N |q_{i}^{truth}|^2}}$$

Дополнительно используются точности удовлетворения законам сохранения:  

$$Lw_1 = \text{mean}\left\(\int_{-\infty}^{+\infty} |q(x,t_i)|^2 dx\right\)$$

$$Lw_2 = \text{mean}\left\(\int_{-\infty}^{+\infty} (q^{\ast}(x,t_i) q_x(x,t_i) - q^{\ast}_x(x,t_i) q(x,t_i)) dx\right\)$$
...
1) Перенос значений: аппроксимация решения похожей задачи, функция потерь имеет вид:  
 
  $$Loss = \frac{\sum\limits_{i=1}^N (|q_{sim}^i-q_{pred}^i|^2)}{N}$$

2) Обычная задача оптимизации для случая PINNs: Обучение уравнению, начальным и граничным условиям, функция потерь имеет вид:
 
  $$Loss = \frac{\sum\limits_{i=1}^N (|q_{0}^i-q_{pred}^i|^2)}{N} + \frac{\sum\limits_{i=1}^N (|Eq(q_{pred}^i)|^2)}{N}$$
...
Отдельно стоит отметить опыт при $\alpha \neq \alpha_0$(fbpinn и enhancement 3 в таблице 4). Как уже отмечалось ранее, этот случай интересен тем, что для него неизвестно аналитическое решение, а численные методы пока не позволяют решать его с высокой точностью. Предложенный подход в совокупности с технологией FBPINN позволяет получить достаточно точное численное решение на больших промежутках времени и согласно ему, солитон в данном случае не затухает (см график 7). Это можно считать практическим результатом данной работы.  
...
В данной работе изучена эффективность методов трансферного обучения в применении к технологии PINNs для решения нелинейного уравнения Шрёдингера. Показано, что использование рассмотренных методов действительно даёт примущество в точности решения и практически не влияет на длительность обучения, при этом наиболее удачным подходом является перенос обучаемых параметров, подробно рассмотренный в работе. Более того, не обнаружено зависимости прироста точности от степени различия коэффициента $\alpha$. Дополнительно рассмотрена задача улучшения уже полученных решений с помощью методов трансферного обучения и получены замечательные результаты для решения уравнения на больших областях, в частности для случая $\alpha \neq \alpha_0$.  
## Ссылки на источники
